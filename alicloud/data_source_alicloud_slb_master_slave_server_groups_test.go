...
func TestAccAlicloudSlbMasterSlaveServerGroupsDataSource_basic(t *testing.T) {
	...
	var existDnsRecordsMapFunc = func(rand int) map[string]string {
		return map[string]string{
			"slb_master_slave_server_groups.#":                       "1",
			"ids.#":                                                  "1",
			"names.#":                                                "1",
			"slb_master_slave_server_groups.0.id":                    CHECKSET,
			"slb_master_slave_server_groups.0.name":                  "tf-testAccslbmasterslaveservergroupsdatasourcebasic",
			"slb_master_slave_server_groups.0.servers.#":             "2",
			"slb_master_slave_server_groups.0.servers.0.weight":      "100",
			"slb_master_slave_server_groups.0.servers.0.instance_id": CHECKSET,
		}
	}

	var fakeDnsRecordsMapFunc = func(rand int) map[string]string {
		return map[string]string{
			"slb_master_slave_server_groups.#": "0",
			"ids.#":                            "0",
			"names.#":                          "0",
		}
	}
